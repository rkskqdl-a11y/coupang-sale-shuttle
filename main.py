...
# 🚀 [System] 사용자 AF7053799 전용 하베스팅 엔진 가동 시작
...
class CoupangUltimateEngine:
    ...
    def fetch_reco_v2(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"   ❌ API 응답 에러: {resp.status_code} | {resp.text[:50]}")
                return []
        exceptException as e:
            return []

    def generate_review(self, product_name):
        """💎 제미나이 1.5 플래시 기반 고품질 칼럼 생성"""
    ...
    def run(self):
        ...
        # 💎 1차 시도: 가장 확실한 Reco v2 엔진 가동
        products = self.fetch_reco_v2()
        success_count, max_target = 0, 10

        if not products:
            print("❌ 상품 데이터 확보 실패. API 권한 또는 키 상태를 점검하세요.")
            return
...
if __name__ == "__main__":
    # 💎 0.1%의 오차도 허용하지 않는 except 구문 교정 완료
    try:
