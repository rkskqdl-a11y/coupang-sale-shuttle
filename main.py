...
def generate_ai_content(product_name):
    """💎 1,500자 이상의 초장문 리뷰 생성 (해요체)"""
    try:
        model = genai.GenerativeModel('gemini-1.5-flash')
        prompt = f"상품명 '{product_name}'에 대해 쇼핑 전문가처럼 친절한 해요체로 1,000자 이상 상세 분석 리뷰를 작성해줘. <h3> 태그를 써서 섹션을 나누고 HTML만 사용해. '할인'이나 '세일' 언급은 금지."
        response = model.generate_content(prompt)
        return response.text.replace("\n", "<br>")
    except:
        return f"<h3>🔍 전문가 분석</h3>{product_name}은 품질과 디자인이 돋보이는 최고의 선택입니다."
...
def fetch_data(keyword):
    """💎 페이지 탐색 없이 1페이지에서 대량 수집 (검증된 방식)"""
    try:
        DOMAIN = "https://api-gateway.coupang.com"
        path = "/v2/providers/affiliate_open_api/apis/openapi/v1/products/search"
        # 1페이지 상단 20개 상품을 훑습니다.
...
def main():
    ...
    # 💎 무조건 결과가 쏟아지는 씨앗 키워드
    seeds = ["티셔츠", "운동화", "세탁기", "건조기", "린넨셔츠", "가습기", "커피머신", "모니터", "단백질보충제", "샴푸", "물티슈", "기저귀", "양말", "베개", "보조배터리"]
    
    existing_ids = {f.split('_')[-1].replace('.html', '') for f in os.listdir("posts") if '_' in f}
    success_count, max_target = 0, 10
    
    print(f"🕵️ 현재 {len(existing_ids)}개 노출 중. 새 상품 {max_target}개 발행 시작!")

    # 💎 1페이지 결과를 순차적으로 처리 (사용자님 요청사항 반영)
    attempts = 0
    while success_count < max_target and attempts < 30:
        attempts += 1
        target = random.choice(seeds)
        print(f"🔄 [{attempts}차] '{target}' 1페이지 탐색 중...")
        
        products = fetch_data(target)
        if not products:
            print("   ⚠️ API에서 데이터를 가져오지 못했습니다. 인증을 확인하세요.")
            continue
        ...
        for item in products:
            ...
            print(f"   ✨ 발견! [{success_count+1}/10] {p_name[:20]}... 발행 시작")
            ...
            success_count += 1
            time.sleep(35) # 안전 발행을 위한 대기
            if success_count >= max_target: break

    # 💎 [중요: 사이트맵 네임스페이스 해결] 구글 표준 규격 삽입
    files = sorted([f for f in os.listdir("posts") if f.endswith(".html")], reverse=True)
    now_iso = datetime.now().strftime("%Y-%m-%d")
    
    # xmlns 선언을 정확히 추가하여 구글 검색 엔진이 인식하게 합니다.
