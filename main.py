...
def generate_ai_content(product_name):
    ...
    prompt = f"상품 '{product_name}'에 대해 전문적인 분석 칼럼을 1,000자 이상 장문으로 작성해줘. <h3> 태그를 활용해 섹션을 나누고 HTML만 사용해. 친절한 '해요체'로 작성하고 '할인' 언급은 절대 금지."
    try:
...
def main():
    ...
    # 💎 [무차별 수집용 게이트웨이 키워드] 카테고리 전체를 훑습니다.
    gateways = ["가전디지털", "패션의류", "생활용품", "주방용품", "뷰티", "스포츠레저", "홈인테리어", "완구취미"]
    
    existing_ids = {f.split('_')[-1].replace('.html', '') for f in os.listdir("posts") if '_' in f}
    success_count, max_target = 0, 10
    
    print(f"🕵️ 현재 {len(existing_ids)}개 노출 중. 쿠팡 전 상품 순차 하베스팅 시작!")

    # 💎 무작위 카테고리 선택 후, 해당 카테고리를 1페이지부터 저인망식으로 훑음
    category = random.choice(gateways)
    print(f"🚀 이번 타겟 카테고리: [{category}]")

    for page in range(1, 51): # 1페이지부터 50페이지까지 순차 수색
        if success_count >= max_target: break
        
        print(f"   🔍 [페이지 {page}] 분석 중...")
        products = fetch_data(category, page)
