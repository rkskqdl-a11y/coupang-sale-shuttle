...
def generate_ai_content(product_name):
    """💎 속도를 위해 글자 수를 최적화한 전문 리뷰 생성"""
    if not GEMINI_KEY: return "분석 데이터 준비 중"
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_KEY}"
    # 응답 속도를 높이기 위해 800자 내외로 요청합니다.
    prompt = f"상품 '{product_name}'에 대해 전문적인 제품 분석 칼럼을 800자 이상 장문으로 작성해줘. <h3> 태그를 활용해 섹션을 나누고 HTML만 사용해. '할인' 언급 절대 금지."
    ...
    return f"<h3>🔍 제품 상세 분석</h3>{product_name}은 뛰어난 가치와 실용성을 갖춘 모델입니다."

def fetch_data(keyword, page):
    """💎 인증 오류 방지를 위한 정렬된 파라미터 호출"""
    try:
        DOMAIN = "https://api-gateway.coupang.com"
        path = "/v2/providers/affiliate_open_api/apis/openapi/v1/products/search"
        params = {"keyword": keyword, "limit": 40, "page": page} # 넉넉하게 수집
        ...
        response = requests.get(f"{DOMAIN}{path}?{query_string}", headers=headers, timeout=10)
...
def main():
    ...
    # 💎 무조건 결과가 나오는 '마르지 않는' 키워드 풀
    seeds = ["양말", "반팔티", "청바지", "슬랙스", "수건", "베개", "멀티탭", "충전기", "케이블", "텀블러", "후라이팬", "냄비", "샴푸", "비누", "칫솔", "치약", "비타민", "유산균", "마스크", "이어폰", "백팩", "보조배터리", "물티슈", "기저귀"]
    ...
    # 💎 목표 달성 시까지 무한 탐색 (페이지 범위를 줄여 속도 확보)
    while success_count < max_target:
        target = random.choice(seeds)
        page = random.randint(1, 15) # 결과가 확실한 앞쪽 페이지 공략
        ...
        for item in products:
            ...
            # AI 글쓰기 시작 로그
            print(f"   ✍️ [{success_count+1}/10] {p_name[:20]}... 발행 중")
            ...
            success_count += 1
            time.sleep(20) # 발행 보장을 위해 대기 시간을 약간 줄임
            if success_count >= max_target: break

    # 💎 [사이트맵 오류 해결] 네임스페이스 삽입 및 강제 업데이트
    files = sorted([f for f in os.listdir("posts") if f.endswith(".html")], reverse=True)
    now_iso = datetime.now().strftime("%Y-%m-%d")
    
    # xmlns 속성을 정확히 선언하여 구글 경고를 제거합니다.
    ...
    print(f"🏁 작업 종료! 총 {len(files)}개 상품 노출 중.")
