...
def main():
    ...
    # 💎 [방대한 카탈로그 시드] 특정 단어가 아닌, 전 품목을 포함하는 대분류 단어
    broad_seeds = ["생활용품", "가전", "주방", "패션", "뷰티", "스포츠", "인테리어", "반려동물", "완구", "디지털", "잡화"]
    
    existing_ids = {f.split('_')[-1].replace('.html', '') for f in os.listdir("posts") if '_' in f}
    success_count, max_target = 0, 10
    
    print(f"🕵️ 현재 {len(existing_ids)}개 노출 중. 쿠팡 전 상품 무작위 수확 시작!")

    attempts = 0
    # 💎 10개를 채울 때까지 키워드와 페이지를 바꿔가며 끈질기게 수집
    while success_count < max_target and attempts < 50:
        attempts += 1
        target = random.choice(broad_seeds)
        page = random.randint(1, 50) # 1~50페이지 사이를 무작위로 타격
        
        print(f"🔄 [{attempts}차] [{target}] p.{page} 수색 중...")
        ...
        for item in products:
            ...
            print(f"   ✨ 발견! [{success_count+1}/10] {p_name[:25]}...")
