...
# 🚀 [System] AF7053799 전용 고품질 & 법적 규정 준수 엔진 가동
print("🚀 [System] 로봇(robots.txt) 동기화 및 대가성 문구 삽입 모드 가동...")

class CoupangFinalMaster:
    def __init__(self):
    ...
    def generate_rich_content(self, item):
        ...
        prompt = (
            f"상품 '{p_name}'(가격: {price}원)에 대해 쇼핑 전문가가 작성한 1,500자 이상의 정밀 분석 글을 써줘.\n\n"
            f"⚠️ 주의사항: 제목에 있는 단어들을 첫 문단에서 그대로 반복하지 마세요.\n"
            f"<h3> 태그를 사용하여 '디자인의 특징', '성능 및 활용도', '가성비 분석', '추천 대상' 4개 섹션으로 구성하세요.\n"
    ...
    def get_real_title(self, path):
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                ...
                return match.group(1)[:35] + "..." if match else "추천 상품"
    ...
    def run(self):
        ...
        print(f"🕵️ 현재 {len(existing_ids)}개 진열 중. '{target}' 고품질 수집 시작!")
    ...
    def update_web(self):
        ...
        with open("index.html", "w", encoding="utf-8") as f:
            ...
            for file in files[:100]:
                ...
                f.write(f"<a class='card' href='posts/{file}'><div>{title}</div><div style='color:#e44d26; font-weight:bold; margin-top:15px;'>전문 분석 읽기 ></div></a>")
            f.write("</div></body></html>")
        print(f"🏁 모든 파일(robots, sitemap, index) 동기화 완료!")

if __name__ == "__main__":
    CoupangFinalMaster().run()
